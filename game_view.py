    ...
    def Step(self):
        ...
        for contact in self.contacts:
            ...
            if bullet:
                ...
                if target.userData is not None:
...
class GameView(ui.RootElement):
    def __init__(self):
        ...
        self.backdrop_texture = drawing.texture.Texture("starfield.png", width=1280, height=720)
        self.ground_texture = drawing.texture.Texture("dirt.png")
        self.backdrop = drawing.Quad(
            globals.backdrop_buffer, tc=numpy.array([(0, 0), (0, 4), (5, 4), (5, 0)])
        )
        ...
        #                       tc = dirt_tc)
        self.walls = [
            actors.StaticBox(self.physics, bl=Point(0, 0), tr=Point(1, self.absolute.size.y)),
            actors.StaticBox(
                self.physics,
        ...
        for i in xrange(1, len(self.land_heights)):
            ...
            if bottom_y > top_y:
                self.landscape.append(
                    actors.StaticTriangle(
                        self.physics, (Point(bottom_x, top_y), Point(top_x, top_y), Point(bottom_x, bottom_y))
                    )
                )
                self.landscape.append(
                    actors.StaticBox(
                        self.physics, bl=Point(bottom_x, -globals.screen.y), tr=Point(top_x, top_y), tc=True
                    )
                )
                ...
                self.landscape.append(
                    actors.StaticTriangle(
                        self.physics, (Point(bottom_x, bottom_y), Point(top_x, bottom_y), Point(top_x, top_y))
                    )
                )
                self.landscape.append(
                    actors.StaticBox(
    ...
    def Draw(self):
        ...
        glDrawElements(
            GL_TRIANGLES, globals.ground_buffer.current_size, GL_UNSIGNED_INT, globals.ground_buffer.indices
        )
        ...
        glDrawElements(
            GL_QUADS, globals.quad_buffer.current_size, GL_UNSIGNED_INT, globals.quad_buffer.indices
        )
        ...
        glDrawElements(
            GL_QUADS,
            globals.nonstatic_text_buffer.current_size,
            GL_UNSIGNED_INT,
            globals.nonstatic_text_buffer.indices,
        )
    ...
    def AddTrex(self):
        ...
        self.enemies.append(type(self, self.physics, bl=bl, tr=bl + size))
