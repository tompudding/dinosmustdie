...
class Intro(Mode):
    ...
    def __init__(self, parent):
        ...
        self.handlers = {
            IntroStages.STARTED: self.Startup,
    ...
    def TextDraw(self, t):
        if not self.skipped_text and self.elapsed < len(self.menu_text.text) * self.letter_duration:
            ...
            self.parent.viewpos.SetTarget(
                self.parent.ship.GetPos() - (globals.screen * 0.5), t, rate=0.4, callback=self.Scrolled
            )
...
class GameMode(Mode):
    def __init__(self, parent):
        ...
        self.tutorial_handlers = {
            ShipStates.TUTORIAL_MOVEMENT: self.TutorialMovement,
        ...
        for i in xrange(15):
            bl = Point(
                random.random() * self.parent.absolute.size.x * 0.9,
                self.parent.max_floor_height + random.random() * 400,
            )
            self.ooze_boxes.append(
                actors.Crate(
                    self.parent.physics,
                    bl=bl,
                    tr=bl + Point(50, 50),
    ...
    def EndTutorial(self):
        self.parent.ship.SetText(
            "Great! Now to get evolution started, lift some crates of primordial goo into the air and destroy them!",
            wait=0,
            limit=6000,
        )
    ...
    def BoxDestroyed(self, box):
        ...
        if p.y < target:
            self.parent.ship.SetText(
                "That was too low by %2.f metres, try again but higher!" % (target - p.y), wait=0, limit=6000
            )
            return
        self.num_boxes -= 1
        if self.num_boxes > 0:
            self.parent.ship.SetText(
                "Good! %d box%s left!" % (self.num_boxes, "" if self.num_boxes == 1 else "es"),
                wait=0,
                limit=4000,
            )
        else:
            self.parent.ship.SetText(
                "Great. Press <space> to wait 3 billion years for life to evolve", wait=0
            )
...
class Titles(Mode):
    blurb = "After waiting for 3 billion years you are starting to seriously fancy some gloopy {gloop}. Unfortunately for you (and the galaxy) you have overslept by 100 million years and without your guiding tentacle Dinosaurs have risen to dominate the planet. Dinosaurs *hate* {gloop}, and so...".format(
        gloop=gloop_name
    )
    ...
    def Startup(self, t):
        ...
        self.parent.viewpos.SetTarget(self.view_target, t, rate=0.4, callback=self.Scrolled)
        self.parent.ship.Disable()
    ...
    def TextDraw(self, t):
        if not self.skipped_text:
            ...
            self.parent.viewpos.SetTarget(
                self.parent.ship.GetPos() - (globals.screen * 0.5), t, rate=0.4, callback=self.ScrolledDown
            )
            # self.parent.viewpos.Follow(t,self.parent.ship)
...
class GameOver(Mode):
    ...
    def __init__(self, parent, win, score):
        ...
        self.parent.Pause()
        self.parent.ship.Disable()
        pygame.mixer.music.load(globals.pyinst.path("end_fail.mp3"))
